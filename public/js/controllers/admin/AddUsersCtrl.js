var app = angular.module('AddUsersCtrl',[])
.controller('AddUsersCtrl', function($http,$scope, $rootScope,$location,commonservice,webapis){
...
$scope.allUserOfSchool  = []
$scope.getAllUsersOfSchool = function()
{
  var apiUrl = webapis.getAllUsersOfSchool($rootScope.userinfo.info.user_id,$scope.selected_school_obj.school_code);
  
 $scope.progress_bar_school = true;
  $http.get(apiUrl).then(function(response){
  var users = response.data;
           if(users.status=="success"){
              $scope.allUserOfSchool = users.data;
